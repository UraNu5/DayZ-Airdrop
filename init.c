...
{	
	...
	float AirPlaneHeight = 400; // Airplane fly height 
	...
	void SpawnAirPlaneAndTeleportPlayer_DEBUG()
	{
        ...
        m_AirPlane.PlaySoundLoop("powerGeneratorLoop", 10000, true); // Attach airplane sound to itself
		...
		if (!RayReady)
		{
			...
			if (GetVelocity(PhysicsBody)[0] < 0.0001 && GetVelocity(PhysicsBody)[1] < 0.0001 && GetVelocity(PhysicsBody)[2] < 0.0001)
			{
				...
				if (PrintInformationMessages && !PrintInformationCoordinates)
					...
					SendMessage("Airdrop landed at " + Math.Round( arrCoords.Get(0).ToInt() ) + " " + Math.Round( arrCoords.Get(2).ToInt() ) + "!");
				}	
